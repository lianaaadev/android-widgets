...
| Persistence | Room 2.6.1 (KSP) | `countdown.db`, schema exported to `schemas/` |
...
Both entry points funnel through one function, `bindCountdownWidget` in `widget/CountdownBinding.kt`,
a thin wrapper over the generic `bindWidget` in `:core`:

```
A. From the widget picker           B. From "Add to home screen"
   launcher allocates appWidgetId      requestPinCountdownWidget(context, occasionId)
   → CountdownWidgetConfigActivity     → launcher places the widget
   → user picks an occasion            → fires our PendingIntent
   → setResult(RESULT_OK)              → CountdownPinnedReceiver
          │                                      │
          └──────────────┬───────────────────────┘
                         ▼
            bindCountdownWidget(context, appWidgetId, occasionId)
                         │
...
```
countdown/src/main/java/com/liana/countdown/
  ...
  data/
    Occasion.kt                   Entity and mark list (the accent palette lives in :core)
  ...
  widget/
    ...
    CountdownBinding.kt           Which widget and which key, over :core's binding helpers
    WidgetPrefs.kt                Per-widget DataStore keys
  work/MidnightAlarmScheduler.kt  Daily tick + boot/clock/timezone receiver

countdown/src/test/java/.../CountdownTest.kt    14 tests over the date arithmetic
```

Buttons, the palette and the type scale used to sit in `ui/Components.kt` and `ui/theme/`; they
are now `:core`'s `design/` package, shared with the weight app.
...
![2x2 and 4x2 widgets in dark and light; on a light wallpaper the block inverts to a solid slab of the occasion's colour](../docs/countdown/WidgetSizes_1x.png)

![The four widget states: counting down, the day itself, been and gone, and orphaned](../docs/countdown/WidgetStates_1x.png)

![Cover screen widget at 352 by 339 dp, counting down and on the day itself](../docs/countdown/CoverScreen_1x.png)

### App

| Occasions | Empty state | New occasion | Widget setup |
|:--:|:--:|:--:|:--:|
| <img src="../docs/countdown/Main_1x.png" width="210" alt="Occasion list with a Japan Trip hero card at 7 days"> | <img src="../docs/countdown/Empty_1x.png" width="210" alt="Empty state reading Nothing on the horizon"> | <img src="../docs/countdown/EditOccasion_1x.png" width="210" alt="New occasion form with date, colour and a live widget preview"> | <img src="../docs/countdown/WidgetConfig_1x.png" width="210" alt="Widget setup screen picking which occasion to count"> |
